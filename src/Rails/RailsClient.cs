...
/// <inheritdoc/>
public sealed class RailsClient : IRailsClient
{
    ...
    readonly Lazy<IRailsClientWithRawResponse> _withRawResponse;

    /// <inheritdoc/>
    public IRailsClientWithRawResponse WithRawResponse
    {
        get { return _withRawResponse.Value; }
    }

    /// <inheritdoc/>
    public IRailsClient WithOptions(Func<ClientOptions, ClientOptions> modifier)
    {
...
/// <inheritdoc/>
public sealed class RailsClientWithRawResponse : IRailsClientWithRawResponse
{
#if NET
        ...
        where T : ParamsBase
    {
        ...
        while (true)
        {
            ...
            if (response != null && (++retries > maxRetries || !ShouldRetry(response)))
            {
                ...
                try
                {
                    throw RailsExceptionFactory.CreateApiException(
                        response.StatusCode,
                ...
                {
                    throw new RailsIOException("I/O Exception", e);
                }
                finally
                {
                    response.Dispose();
    ...
    {
        return e is IOException || e is RailsIOException;
    }

    public void Dispose() => this.HttpClient.Dispose();

    public RailsClientWithRawResponse()
    {
        _options = new();
